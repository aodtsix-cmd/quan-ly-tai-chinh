...
**OCR screenshot import** ([src/ocr_import.py](src/ocr_import.py), web-only — see below for why): the user's workflow is to add a clear note to every MoMo/MB Bank transfer, then batch-upload screenshots of transaction history at the end of the day.
- `extract_text(image_file)` runs `pytesseract.image_to_string(..., lang="vie+eng")` — needs the system `tesseract` binary (see Commands), not just the Python package.
- `parse_candidates(text)` is a **deliberately rough, first-pass** line parser (regex for `-50.000đ`-style amounts, `+` prefix ⇒ `direction="in"`), explicitly not trusted on its own — it exists to produce a starting guess for a human to correct, not a final answer. It has not been tuned against a real MoMo/MB Bank screenshot (only a synthetic PIL-generated test image so far). `NON_TRANSACTION_KEYWORDS` (currently just "số dư"/"balance") filters out running-balance lines that would otherwise look like a transaction — extend this list rather than assuming it's complete once real screenshots are tried.
